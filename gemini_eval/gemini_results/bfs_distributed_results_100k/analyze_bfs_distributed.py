...
#         print("Processes\tExecution Time (s)\tAvg CPU Usage (%)\tAvg Disk Utilization (%)\tAvg Memory Usage (%)\tMPI Send Time (s)\tMPI Recv Time (s)")
#         for _, row in results.iterrows():
#             print(f"{row['Processes']}\t{row['Execution Time (s)']:.3f}\t{row['Avg CPU Usage (%)']:.2f}\t{row['Avg Disk Utilization (%)']:.2f}\t{row['Avg Memory Usage (%)']:.2f}\t{row['MPI Send Time (s)']:.3f}\t{row['MPI Recv Time (s)']:.3f}")
#         results.to_csv(os.path.join(output_dir, "bfs_distributed_100m_results.csv"), index=False)
#         print("Results saved to bfs_distributed_100m_results.csv")
#     else:
...
def analyze_bfs_metrics(output_dir):
    ...
    for count in process_counts:
        ...
        disk_file = os.path.join(output_dir, f"iostat_{count}_procs.log")
        memory_file = os.path.join(output_dir, f"sar_{count}_procs.log")
        mpi_file = os.path.join(output_dir, f"mpi_logs_100k_{count}process.log")
...
if __name__ == "__main__":
    ...
    if not results.empty:
        ...
        results.to_csv(os.path.join(output_dir, "bfs_distributed_100k_results.csv"), index=False)
        print("Results saved to bfs_distributed_100k_results.csv")
    else:
        print("No metrics data processed. Please check the log files.")
