...
def analyze_pr_metrics(output_dir):
    results = []
    process_counts = ["2", "4", "8", "16"]

    for count in process_counts:
        pr_file = os.path.join(output_dir, f"pr_{count}_mpi_output.log")
        ...
        mpi_file = os.path.join(output_dir, f"mpi_logs_100m_{count}process.log")

        if not (os.path.exists(pr_file) and os.path.exists(cpu_file) and os.path.exists(disk_file) and os.path.exists(memory_file) and os.path.exists(mpi_file)):
            print(f"Missing files for process count {count}. Skipping...")
            continue

        exec_time = parse_pr_output(pr_file)
...
if __name__ == "__main__":
    ...
    results = analyze_pr_metrics(output_dir)
    if not results.empty:
        print("{:<10} {:<20} {:<20} {:<25} {:<25} {:<20} {:<20}".format(
            "Processes", "Execution Time (s)", "Avg CPU Usage (%)", "Avg Disk Utilization (%)", "Avg Memory Usage (%)", "MPI Send Time (s)", "MPI Recv Time (s)"
        ))
        for _, row in results.iterrows():
            print("{:<10} {:<20.3f} {:<20.2f} {:<25.2f} {:<25.2f} {:<20.3f} {:<20.3f}".format(
                row['Processes'], row['Execution Time (s)'], row['Avg CPU Usage (%)'], row['Avg Disk Utilization (%)'], row['Avg Memory Usage (%)'], row['MPI Send Time (s)'], row['MPI Recv Time (s)']
            ))
        results.to_csv(os.path.join(output_dir, "pr_distributed_100m_results.csv"), index=False)
        print("Results saved to pr_distributed_100m_results.csv")
    else:
