...
public:
  ...
  void gather_vertex_array(T * array, int root) {
    if (partition_id!=root) {
      MPI_Send_Wrapped(array + partition_offset[partition_id], sizeof(T) * owned_vertices, MPI_CHAR, root, GatherVertexArray, MPI_COMM_WORLD);
    } else {
      for (int i=0;i<partitions;i++) {
        if (i==partition_id) continue;
        MPI_Status recv_status;
        MPI_Recv_Wrapped(array + partition_offset[i], sizeof(T) * (partition_offset[i + 1] - partition_offset[i]), MPI_CHAR, i, GatherVertexArray, MPI_COMM_WORLD, &recv_status);
        int length;
  ...
  void load_undirected_from_directed(std::string path, VertexId vertices) {
    ...
    }
    {
      std::thread recv_thread_dst([&](){
        ...
        while (finished_count < partitions) {
          ...
          if (recv_bytes==1) {
            finished_count += 1;
            char c;
            MPI_Recv_Wrapped(&c, 1, MPI_CHAR, i, ShuffleGraph, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            continue;
          }
          assert(recv_bytes % edge_unit_size == 0);
          int recv_edges = recv_bytes / edge_unit_size;
          MPI_Recv_Wrapped(recv_buffer, edge_unit_size * recv_edges, MPI_CHAR, i, ShuffleGraph, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          // #pragma omp parallel for
      ...
      while (read_bytes < bytes_to_read) {
        ...
        for (EdgeId e_i=0;e_i<curr_read_edges;e_i++) {
          ...
          if (buffered_edges[i] == CHUNKSIZE) {
            MPI_Send_Wrapped(send_buffer[i].data(), edge_unit_size * buffered_edges[i], MPI_CHAR, i, ShuffleGraph, MPI_COMM_WORLD);
            buffered_edges[i] = 0;
          }
        }
        for (EdgeId e_i=0;e_i<curr_read_edges;e_i++) {
      ...
      for (int i=0;i<partitions;i++) {
        char c = 0;
        MPI_Send_Wrapped(&c, 1, MPI_CHAR, i, ShuffleGraph, MPI_COMM_WORLD);
      }
    ...
    if (sparse) {
      ...
      std::thread send_thread([&](){
        for (int step=1;step<partitions;step++) {
          ...
          for (int s_i=0;s_i<sockets;s_i++) {
            MPI_Send_Wrapped(send_buffer[partition_id][s_i]->data, sizeof(MsgUnit<M>) * send_buffer[partition_id][s_i]->count, MPI_CHAR, i, PassMessage, MPI_COMM_WORLD);
          }
        }
      });
      std::thread recv_thread([&](){
        for (int step=1;step<partitions;step++) {
          ...
          for (int s_i=0;s_i<sockets;s_i++) {
            ...
            MPI_Recv_Wrapped(recv_buffer[i][s_i]->data, recv_buffer[i][s_i]->count, MPI_CHAR, i, PassMessage, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      ...
      if (dense_selective!=nullptr && partitions>1) {
        ...
        std::thread send_thread([&](){
          for (int step=1;step<partitions;step++) {
            ...
            MPI_Send_Wrapped(dense_selective->data + WORD_OFFSET(partition_offset[partition_id]), owned_vertices / 64, MPI_UNSIGNED_LONG, recipient_id, PassMessage, MPI_COMM_WORLD);
          }
        });
        std::thread recv_thread([&](){
          for (int step=1;step<partitions;step++) {
            int sender_id = (partition_id - step + partitions) % partitions;
            MPI_Recv_Wrapped(dense_selective->data + WORD_OFFSET(partition_offset[sender_id]), (partition_offset[sender_id + 1] - partition_offset[sender_id]) / 64, MPI_UNSIGNED_LONG, sender_id, PassMessage, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          }
        });
        send_thread.join();
        recv_thread.join();
      ...
      std::thread send_thread([&](){
        for (int step=0;step<partitions;step++) {
          ...
          for (int s_i=0;s_i<sockets;s_i++) {
            MPI_Send_Wrapped(send_buffer[i][s_i]->data, sizeof(MsgUnit<M>) * send_buffer[i][s_i]->count, MPI_CHAR, i, PassMessage, MPI_COMM_WORLD);
          }
        }
      });
      std::thread recv_thread([&](){
